...
class TiagoAction
{
    private:
        ...
        {
            // We want the transform from frame base_laser_link to frame map
            // base_laser_link is the laser scanner position
            // map is the pose of an object in the map
            // The laser scanner is mounted on front of the robot
            tf::TransformListener listener;
            tf::StampedTransform transform;
            // Define the target frame (map) and source frame (base_laser_link)
            std::string target_frame = "map";
            std::string source_frame = aLaser_msg->header.frame_id.c_str();// "base_laser_link"
            try
            {
            ...
            ROS_INFO("Transform values: [x: %f, y: %f, z: %f]", transform.getOrigin().getX(), transform.getOrigin().getY(), transform.getOrigin().getZ());
            ...
            for (const Circle& aCircle : my_circles)
            {
                ...
                base_laser_point.z = aCircle.radius;
                ...
                ROS_INFO("Position of cylinder: [x: %f, y: %f], radius: %f", base_link_point.x, base_link_point.y, base_link_point.z);          
            }
    ...
    protected:
        ...
        actionlib::SimpleActionServer<assignment_2_group_27::TiagoAction> as_;
        std::string action_name_;
        assignment_2_group_27::TiagoFeedback feedback_;
        assignment_2_group_27::TiagoResult result_;
    public:
        ...
        void executeCB(const assignment_2_group_27::TiagoGoalConstPtr &goal)
        {
            // Helper variables
            ros::Rate r(1);
            ...
            // Perform obstacle detection only if required
            if (goal->detection == true)
            {
